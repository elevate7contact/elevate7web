...
const PLANS = {
  '30min': {
    amount: 5000,
    name: 'Consultoría 30 minutos — Elevate Zeven',
    calLink: 'https://cal.com/elevate-zeven/30min',
  },
  '60min': {
    amount: 12000,
    name: 'Consultoría 1 hora — Elevate Zeven',
...
function isAllowedOrigin(origin) {
  ...
  if (origin.endsWith('.vercel.app') && origin.includes('elevate-zeven')) return true
...
export default async function handler(req, res) {
  ...
  const cancelUrl = process.env.VERCEL_PROJECT_PRODUCTION_URL
    ? `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL}/#opciones`
    : `${process.env.VITE_SITE_URL || 'https://elevate-zeven-web.vercel.app'}/#opciones`
